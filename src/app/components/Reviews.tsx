...
function Review({ title, body, author, rating, className, ...props }: any) {
  ...
  return (
    ...
    >
      {reviews
        .concat(reviews)
        .map(
          ({ review, reviewIndex }: { review: any; reviewIndex: number }) => (
            <Review
              key={reviewIndex}
...
function ReviewGrid() {
  ...
  let isInView: any = useInView(containerRef, { once: true, amount: 0.4 });
  let columns: any = splitArray({ array: reviews, numParts: 3 });
  columns = [
    columns[0],
    columns[1],
